...
export default function EditCard() {
  ...
  useEffect(() => {
    if (item) {
      setHolder(item.cardHolder || "");
      setNumber(item.cardNumber || "");
      setExpiry(item.expiry || "");
      setCvv(item.cvv || "");
      setCardType((item.cardType as "visa" | "mastercard" | "amex") || "visa");
      setNotes(item.notes || "");
    }
  }, [item]);
  ...
  const handleUpdate = () => {
    ...
    navigate(-1); // Go back to the View Card screen
  };

  if (!item) return <div className="p-8 text-text">Card not found.</div>;
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex justify-between items-center px-8 py-6 border-b border-border shrink-0">
        ...
        <h1 className="text-xl font-bold text-text">Edit Card</h1>
        <button
